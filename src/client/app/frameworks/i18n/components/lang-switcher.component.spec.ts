                ...
                t.fakeAsync(t.inject([TestComponentBuilder], (tcb: TestComponentBuilder) => {
                    return tcb.createAsync(TestComponent)
                        .then(rootTC => {
                            ...
                            t.e(getDOM().querySelectorAll(appDOMEl, '.field > select option').length).toBe(1);
                            t.e(getDOM().querySelectorAll(appDOMEl, '.field > select option')[0].value).toBe('en');
                        });
                })));
        });
                ...
                t.fakeAsync(t.inject([TestComponentBuilder], (tcb: TestComponentBuilder) => {
                    return tcb.createAsync(TestComponent)
                        .then(rootTC => {
                            rootTC.detectChanges();
                            let appDOMEl = rootTC.debugElement.children[0].nativeElement;
                            t.e(getDOM().querySelectorAll(appDOMEl, '.field > select option').length).toBe(3);
                            t.e(getDOM().querySelectorAll(appDOMEl, '.field > select option')[0].value).toBe('en');
                            t.e(getDOM().querySelectorAll(appDOMEl, '.field > select option')[1].value).toBe('es');
                            t.e(getDOM().querySelectorAll(appDOMEl, '.field > select option')[2].value).toBe('ko');
                        });
                })));
        });
    });
